...
RegExp('^\\s*([a-z][a-z-]*)(?:\\s*(=)\\s*((\")[^\"]*(\"|$)|(\')[^\']*(\'|$)|(?=[a-z][a-z-]*\\s*=)|[^\"\'\\s]*))?','i'),ENTITY_RE=/^(#[0-9]+|#x[0-9a-f]+|\w+);/i,splitWillCapture='a,b'.split(/(,)/).length===3,EFLAGS_TEXT=html4.eflags.CDATA|html4.eflags.RCDATA;function
makeSaxParser(handler){return function(htmlText,param){return parse(htmlText,handler,param)}}function
parse(htmlText,handler,param){var h=handler,current,eflags,end,m,next,noMoreEndComments,noMoreGT,p,parts,pos,tag,tagName;h.startDoc&&h.startDoc(param),parts=htmlSplit(htmlText),noMoreGT=false,noMoreEndComments=false;for(pos=0,end=parts.length;pos<end;){current=parts[pos++],next=parts[pos];switch(current){case'\x26':ENTITY_RE.test(next)?(h.pcdata&&h.pcdata('\x26'+next,param),++pos):h.pcdata&&h.pcdata('\x26amp;',param);break;case'\x3c/':(m=/^(\w+)[^\'\"]*/.exec(next))?m[0].length===next.length&&parts[pos+1]==='\x3e'?(pos+=2,tagName=lcase(m[1]),html4.ELEMENTS.hasOwnProperty(tagName)&&(h.endTag&&h.endTag(tagName,param))):(pos=parseEndTag(parts,pos,h,param)):h.pcdata&&h.pcdata('\x26lt;/',param);break;case'\x3c':(m=/^(\w+)\s*\/?/.exec(next))?m[0].length===next.length&&parts[pos+1]==='\x3e'?(pos+=2,tagName=lcase(m[1]),html4.ELEMENTS.hasOwnProperty(tagName)&&(h.startTag&&h.startTag(tagName,[],param),eflags=html4.ELEMENTS[tagName],eflags&EFLAGS_TEXT&&(tag={'name':tagName,'next':pos,'eflags':eflags},pos=parseText(parts,tag,h,param)))):(pos=parseStartTag(parts,pos,h,param)):h.pcdata&&h.pcdata('\x26lt;',param);break;case'\x3c!--':if(!noMoreEndComments){for(p=pos+1;p<end;++p)if(parts[p]==='\x3e'&&/--$/.test(parts[p-1]))break;p<end?(pos=p+1):(noMoreEndComments=true)}noMoreEndComments&&(h.pcdata&&h.pcdata('\x26lt;!--',param));break;case'\x3c!':if(!/^\w/.test(next))h.pcdata&&h.pcdata('\x26lt;!',param);else{if(!noMoreGT){for(p=pos+1;p<end;++p)if(parts[p]==='\x3e')break;p<end?(pos=p+1):(noMoreGT=true)}noMoreGT&&(h.pcdata&&h.pcdata('\x26lt;!',param))}break;case'\x3c?':if(!noMoreGT){for(p=pos+1;p<end;++p)if(parts[p]==='\x3e')break;p<end?(pos=p+1):(noMoreGT=true)}noMoreGT&&(h.pcdata&&h.pcdata('\x26lt;?',param));break;case'\x3e':h.pcdata&&h.pcdata('\x26gt;',param);break;case'':break;default:h.pcdata&&h.pcdata(current,param)}}h.endDoc&&h.endDoc(param)}function
...
parseEndTag(parts,pos,h,param){var tag=parseTagAndAttrs(parts,pos);return tag?(tag.eflags!==void
0&&(h.endTag&&h.endTag(tag.name,param)),tag.next):parts.length}function parseStartTag(parts,pos,h,param){var
tag=parseTagAndAttrs(parts,pos);if(!tag)return parts.length;if(tag.eflags!==void
0){h.startTag&&h.startTag(tag.name,tag.attrs,param);if(tag.eflags&EFLAGS_TEXT)return parseText(parts,tag,h,param)}return tag.next}endTagRe={};function
parseText(parts,tag,h,param){var end=parts.length,buf,first,p,re;endTagRe.hasOwnProperty(tag.name)||(endTagRe[tag.name]=new
RegExp('^'+tag.name+'(?:[\\s\\/]|$)','i')),re=endTagRe[tag.name],first=tag.next,p=tag.next+1;for(;p<end;++p)if(parts[p-1]==='\x3c/'&&re.test(parts[p]))break;p<end&&(p-=1),buf=parts.slice(first,p).join('');if(tag.eflags&html4.eflags.CDATA)h.cdata&&h.cdata(buf,param);else
if(tag.eflags&html4.eflags.RCDATA)h.rcdata&&h.rcdata(normalizeRCData(buf),param);else
throw new Error('bug');return p}function parseTagAndAttrs(parts,pos){var m=/^(\w+)/.exec(parts[pos]),tag={'name':lcase(m[1])},aName,aValue,abuf,attrs,buf,end,p,quote,sawQuote;html4.ELEMENTS.hasOwnProperty(tag.name)?(tag.eflags=html4.ELEMENTS[tag.name]):(tag.eflags=void
0),buf=parts[pos].substr(m[0].length),p=pos+1,end=parts.length;for(;p<end;++p){if(parts[p]==='\x3e')break;buf+=parts[p]}if(end<=p)return;attrs=[];while(buf!==''){m=ATTR_RE.exec(buf);if(!m)buf=buf.replace(/^[\s\S][^a-z\s]*/,'');else
...
aName=lcase(m[1]),aValue=m[2]?decodeValue(m[3]):aName,attrs.push(aName,aValue),buf=buf.substr(m[0].length)}return tag.attrs=attrs,tag.next=p+1,tag}function
...
sanitizeAttribs(tagName,attribs,opt_naiveUriRewriter,opt_nmTokenPolicy){var attribKey,attribName,atype,i,sanitizedDeclarations,value;for(i=0;i<attribs.length;i+=2){attribName=attribs[i],value=attribs[i+1],atype=null,((attribKey=tagName+'::'+attribName,html4.ATTRIBS.hasOwnProperty(attribKey))||(attribKey='*::'+attribName,html4.ATTRIBS.hasOwnProperty(attribKey)))&&(atype=html4.ATTRIBS[attribKey]);if(atype!==null)switch(atype){case
html4.atype.NONE:break;case html4.atype.SCRIPT:value=null;break;case html4.atype.STYLE:if('undefined'===typeof
parseCssDeclarations){value=null;break}sanitizedDeclarations=[],parseCssDeclarations(value,{'declaration':function(property,tokens){var
normProp=property.toLowerCase(),schema=cssSchema[normProp];if(!schema)return;sanitizeCssProperty(normProp,schema,tokens,opt_naiveUriRewriter),sanitizedDeclarations.push(property+': '+tokens.join(' '))}}),value=sanitizedDeclarations.length>0?sanitizedDeclarations.join(' ; '):null;break;case
...
html4.atype.URI:value=safeUri(value,opt_naiveUriRewriter);break;case html4.atype.URI_FRAGMENT:value&&'#'===value.charAt(0)?(value=value.substring(1),value=opt_nmTokenPolicy?opt_nmTokenPolicy(value):value,value!==null&&value!==void
0&&(value='#'+value)):(value=null);break;default:value=null}else value=null;attribs[i+1]=value}return attribs}function
makeTagPolicy(opt_naiveUriRewriter,opt_nmTokenPolicy){return function(tagName,attribs){if(!(html4.ELEMENTS[tagName]&html4.eflags.UNSAFE))return sanitizeAttribs(tagName,attribs,opt_naiveUriRewriter,opt_nmTokenPolicy)}}function
